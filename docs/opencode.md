...
- Bridge no ar (`https://rblxmcp.onrender.com`, ou local) — confira com
  `https://rblxmcp.onrender.com/health` → `{"ok":true}`
...
```powershell
$BASE = "https://rblxmcp.onrender.com"
...
```json
{
  ...
  "mcp": {
    "roblox-bridge": {
      "type": "remote",
      "url": "https://rblxmcp.onrender.com/mcp/sse",
      "enabled": true,
      "oauth": false,
      "headers": {
...
- **Vários Studios**: cada um com seu token e sua própria conexão MCP (o servidor isola as sessões por padrão).
