-- StudioMcpBridge.server.lua
-- MVP de teste: coloque este Script em ServerScriptService no Studio (modo Edit).
-- Ele faz long-poll no seu site (bridge) e executa comandos da IA.
-- Requer: HttpService.HttpEnabled = true
-- Configure os Attributes do Script: BRIDGE_URL e TOKEN
--
-- Fluxo (ver diagrama): CLI de IA -> Seu site (fila Redis) -> este script (Usuário A/B)

local HttpService = game:GetService("HttpService")

local SCRIPT = script
local BRIDGE_URL = SCRIPT:GetAttribute("BRIDGE_URL") or "http://127.0.0.1:3000"
local TOKEN = SCRIPT:GetAttribute("TOKEN") or "SEU_TOKEN_DE_SESSAO"
...
local function resolvePath(path: string): Instance?
	...
	if first == "game" then
		...
		current = ok and svc or game:FindFirstChild(first)
		table.remove(parts, 1)
	end
...
local function execCommand(cmd)
	...
	elseif t == "set_property" then
		...
		if type(prop) ~= "string" or prop == "" then error("property inválida") end
		if string.lower(prop) == "source" then error("use set_script_source p/ alterar Source") end
		obj[prop] = p.value
		return { path = obj:GetFullName(), property = prop, value = p.value }
	...
	elseif t == "clone_object" then
		...
		if p.parent and p.parent ~= "" then
			...
			if not parent then error("parent destino não encontrado: " .. tostring(p.parent)) end
		end
		local c = obj:Clone()
		c.Parent = parent
...
local function pollOnce()
	...
	if res.StatusCode ~= 200 then
		warn("[mcp-bridge] poll HTTP " .. res.StatusCode)
		task.wait(3)
		return
	end
	local body = HttpService:JSONDecode(res.Body)
	local cmd = body and body.command
	if not cmd then return end -- timeout, normal: faz poll de novo

	local ok, data = pcall(execCommand, cmd)
	local payload = ok
	...
	print("[mcp-bridge] comando " .. cmd.type .. " -> " .. (ok and "ok" or "erro"))
end

print("[mcp-bridge] online. URL=" .. BRIDGE_URL)
SCRIPT:GetAttributeChangedSignal("TOKEN"):Connect(function()
	TOKEN = SCRIPT:GetAttribute("TOKEN")
	print("[mcp-bridge] token atualizado")
end)

while true do
	local ok, err = pcall(pollOnce)
	if not ok then
		warn("[mcp-bridge] " .. tostring(err))
		task.wait(3)
	end
end
